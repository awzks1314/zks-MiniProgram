// miniprogram/pagesOther/physique/physique.js
Page({
  ...
  weightStandardCalculate: function () {
    let weight = 0;
    let height = this.data.height / 100;
    weight = (this.data.STANDARD * (height * height)).toFixed(1);
    this.setData({
  ...
  physicalConditionCalculate: function () {
    let rule = this.data.rules[this.data.index];
    let value = 0;
    ...
    this.setData({
      physicalCondition: this.data.ruleConfig[value]
    })

    this.setData({
      danger: this.data.dangerConfig[value]
    })
  },
  /**
   * 用户点击右上角分享
