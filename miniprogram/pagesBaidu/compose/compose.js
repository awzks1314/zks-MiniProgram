...
Page({
  ...
  search(){
    ...
    var that = this
    wx.request({
      url: 'https://aip.baidubce.com/rpc/2.0/creation/v1/poem?access_token='+wx.getStorageSync('accessToken'),
      data:{
      ...
      success: (result) => {
        if(result.data.poem){
          result.data.poem.forEach(e => {
              e.list = e.content.split(' ')
          });
          that.setData({
            poem:result.data.poem
          })
        }else{
          app.toast('重试或联系开发人员~')
        }
